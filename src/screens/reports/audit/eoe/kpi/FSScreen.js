...
const FSScreen = ({ itemMain, data, isUploaded = false, isRefreshData = false, callBackData }) => {
    ...
    const handlerInputValue = (type, item, index) => {
        ...
        // 
        input.type = type
        input.keyValue = (type == 'HBL') ? 'HBLValue' : 'CompetitorValue'
        input.item = item
        input.index = index
        setMutate(e => !e)
        //
    ...
    const renderItem = ({ item, index }) => {
        ...
        const isEditHBL = input.type == 'HBL' && input.item.ProductId == item.ProductId
