...
import { View, StyleSheet, ActivityIndicator, Platform, KeyboardAvoidingView, TouchableOpacity } from "react-native";
import { SafeAreaView } from 'react-native-safe-area-context';
import { useSelector } from "react-redux";
import useTheme from "../../../../hooks/useTheme";
import { groupDataByKey, handlerGoBack, alertConfirm, removeVietnameseTones } from "../../../../utils/helper";
...
import ViewPictures from "../../../../components/gallary/ViewPictures";
import _ from 'lodash';

const Default_SurveyReportScreen = ({ navigation }) => {
    const { appColors, styleDefault } = useTheme();
    const { shopInfo } = useSelector(state => state.shop)
    const { menuReportInfo } = useSelector(state => state.menu)
    const [pageConfig, setPageConfig] = useState({ "pageMain": { "INPUT": 1 }, "viewTabGroup": false })
    ...
    const LoadData = async () => {
        ...
        await REPORT_API.GetDataByShop(item, async (mData, _mPhoto, lastupdate, isUploaded, _isLocked) => {
            ...
            await setDataMainSurvey(arr)
            await setDataSurvey(arr)
            await setDataPhoto(photo)
            await setLastUpdate(lastupdate)
            await setIsUploaded(isUploaded == 1)
        })
        await setPageConfig(JSON.parse(menuReportInfo.reportItem || '{}'))
        //
            ...
            await REPORT_CONTROLLER.GetDataReportUpload(info, async (mData) => {
                ...
                await REPORT_API.UploadDataReport(mData, async (result) => {
                    if (result.statusId === 200) {
                        LoadData()
                        toastSuccess("Thông báo", "Đã gửi báo cáo, Hệ thống đang tiếp tục đẩy file hình ảnh/ghi âm, Bạn vẫn làm việc bình thường ")
                    } else {
                        toastError(`Lỗi  ${result.statusId}`, result.messager)
                    }
    ...
    // Action
    const onShowPicture = (item) => {
        pictureShow.visible = true
        pictureShow.index = 0
        pictureShow.dataPhoto = [{ url: item.ImageRoot, photoType: item.ItemName }]
        setMutate(e => !e)
    }
    const onHidePicture = () => {
    ...
    const onRenewReport = () => {
        alertConfirm("Thực hiện lại báo cáo", "Bạn muốn xoá toàn bộ dữ liệu & nhập lại từ đầu?", () => {
            REPORT_CONTROLLER.DeleteReportbyId(shopInfo.shopId, menuReportInfo.id, shopInfo.auditDate).then(rs => {
                if (rs.isSuccess == true) {
                    LoadData()
                    toastSuccess("Đã xoá", "Đang tải lại dữ liệu");
                } else {
                    toastError("Xoá", rs.message)
                }
            })
        })
    }
    ...
    const styles = StyleSheet.create({
        ...
        itemMain: { flex: 1, flexWrap: 'wrap', paddingHorizontal: 8 },
        itemContainer: { width: '100%', borderRadius: 5, alignSelf: 'center', margin: 3, backgroundColor: appColors.lightColor },
        ...
        contentItemTab: { flex: 1, backgroundColor: appColors.lightColor, marginTop: 0, paddingStart: 5, paddingEnd: 5 },
        ...
        titleGroup: { width: '100%', fontWeight: 'bold', color: appColors.textColor, fontSize: 14, padding: 8 },
    })
    const renderItem = ({ item, index }) => {
        const onImagePress = () => onShowPicture(item)
        const isFollowItem = _.filter(dataSurvey, (e) => item.ItemValidation == e.Id && e.AnswerValue?.toLowerCase().startsWith('không')).length > 0
        item.Required = isFollowItem ? 0 : 1
        return (
            <View key={`index_ite_${index}`} style={styles.itemMain}>
                {item.isParent && !pageConfig?.viewTabGroup && <Text style={styles.titleGroup}>{item.GroupName}</Text>}
                <View style={styles.itemContainer}>
                    {isValidField(item.ImageRoot) &&
                        <TouchableOpacity style={styles.imageStyle} onPress={onImagePress}>
                            <Image
    ...
    const renderItemTab = (item) => {
        ...
        return (
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.contentItemTab}>
                <SearchData
                    visible={item.isSearchData || false}
                    value={search.text}
    ...
    return (
        <SafeAreaView style={{ flex: 1 }}>
            ...
            <Header
                ...
                onLeftPress={onBack}
                onMiddlePress={!isUploaded && onRenewReport}
                iconNameMiddle={"refresh-sharp"}
            ...
            <View style={styleDefault.contentMain}>
                ...
                {isQuestion == 1 ?
                    ...
                    :
                    <CapturePhotoList navigation={navigation} data={dataPhoto} status={isUploaded} />
                }
            </View>
...
export default Default_SurveyReportScreen;
